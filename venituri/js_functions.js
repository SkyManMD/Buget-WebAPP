	...
	{	
		...
		var chbox_venituri = new Array();
		var k=0,m=0,sum_min,sum_max,date_min,date_max;
		...
		if (chbox[i].checked && chbox[i].name=='venit') chbox_venituri[m++]=chbox[i].value;
		if (chbox[i].name=='sum_min') 
			if(chbox[i].value=="") sum_min="valoare_venit"; else sum_min="'"+chbox[i].value+"'";
		if (chbox[i].name=='sum_max') 
			if(chbox[i].value=="") sum_max="valoare_venit"; else sum_max="'"+chbox[i].value+"'";
		if (chbox[i].name=='date_min') 
			if(chbox[i].value=="") date_min="data_inr_venit"; else date_min="'"+chbox[i].value+"'";
		if (chbox[i].name=='date_max') 
			if(chbox[i].value=="") date_max="data_inr_venit"; else date_max="'"+chbox[i].value+"'";
		}
		...
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.send("chbox_localit="+chbox_localitati+"&chbox_venituri="+
					  chbox_venituri+"&sum_min="+sum_min+"&sum_max="+sum_max+
					  "&date_min="+date_min+"&date_max="+date_max); //tabelurile se trimit sub forma de string;
	}
