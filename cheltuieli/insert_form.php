<?php
	...
	$sel_categorie=mysql_query('SELECT * FROM categorii_cheltuieli');
	$sel_localitate=mysql_query('SELECT id_localitate,nume_localitate FROM localitate ORDER BY nume_localitate ASC');
	$sel_institutie=mysql_query('SELECT * FROM institutie_chelt');
	...
	print"<div class='select_cl'><select name='institutie'><option selected disabled>Alege Instituția</option>";
	while($array=mysql_fetch_array($sel_institutie))
	{
		print "<option value='$array[0]'>".$array[1].'</option>';
	}
	print"</select> </div><br/> <br/>";
	...
	while($array=mysql_fetch_array($sel_categorie))
	{
		...
		$sel_category_items=mysql_query("SELECT id_cheltuiala,nume_cheltuiala FROM cheltuieli WHERE id_cat_cheltuieli='$array[0]' ORDER BY nume_cheltuiala ASC"); //selectare subcategorii pentru fiecare categorie;
