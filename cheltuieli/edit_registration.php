<html> 
<head>
		<title>Editare : Cheltuieli</title>
...
<?php
	...
	$id_chelt=(int)mysql_real_escape_string($_GET['ID_for_edit']);
	
	$select_q=mysql_query("SELECT data_inr_cheltuiala,valoare_cheltuiala FROM buget_cheltuieli
						   WHERE id_buget_cheltuieli='$id_chelt'");
	...
	if(isset($_POST['ok_edit_btn']))
	{
		...
		if($data && $sum_val)
		$edit_q=mysql_query("UPDATE buget_cheltuieli
							 SET data_inr_cheltuiala='$data', valoare_cheltuiala='$sum_val'
							 WHERE id_buget_cheltuieli='$id_chelt'");
